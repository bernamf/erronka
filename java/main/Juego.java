...
public class Juego {
	...
	private static void torneo(Scanner scan, ArrayList<Caballero> caballeros, ModeloCaballero modeloCaballero) {
		...
		for (int i = 0; i < 3; i++) {
			...
			//conbate el 1 contra 
			ganador=peleaDeCaballeros(caballeros, modeloCaballero, participantesTorneo.get(i), participantesTorneo.get(i+4));
		...
		for (int i = 0; i < 1; i++) {
			...
			ganador=peleaDeCaballeros(caballeros, modeloCaballero, participantesTorneo.get(i), participantesTorneo.get(i+2));
			if (ganador==1) {
				Visor.mostrarMensaje(participantesTorneo.get(i)+" pasa a la siguiente ronda");
				idparticipantesFuera.add(i+4);
			}else if (ganador==2) {
		...
		if (peleaDeCaballeros(caballeros, modeloCaballero, participantesTorneo.get(0), participantesTorneo.get(1))==1) {
			Visor.mostrarMensaje("--------------------------------------------------------------");
			Visor.mostrarMensaje(participantesTorneo.get(0).getNombre()+" SE CONVIERTE EN EL GLORIOSO GANADOR DEL TORNEO");
			Visor.mostrarMensaje("--------------------------------------------------------------");
			
		}else if (peleaDeCaballeros(caballeros, modeloCaballero, participantesTorneo.get(0), participantesTorneo.get(1))==2) {
			Visor.mostrarMensaje("--------------------------------------------------------------");
			Visor.mostrarMensaje(participantesTorneo.get(1).getNombre()+" SE CONVIERTE EN EL GLORIOSO GANADOR DEL TORNEO");
			Visor.mostrarMensaje("--------------------------------------------------------------");
		}else {
			Visor.mostrarMensaje("Debido a terminar en empate, la moneda decidira su destino");
			if ((Math.random()*(1-0+1)+0)==1) {
